import { use, useEffect, useState } from "react";
import { Link } from "react-router-dom";
import axios from 'axios';

export default function Header( props ){

    // [2] 로그인 상태 저장하는 상태변수, 
    const [login,setLogin] = useState(false); // 초기값은 false 로그인 안했다는 뜻 

    // [3] 로그인 중인 회원 정보 담는 상태 변수 
    const [user,setUser] = useState(null); // 초기값은 비로그인 정보 



    // [1] 로그인 상태에 따라 메뉴 분기 함수
    const getMyInfo = async()=>{
        //1) 로그인시 localStorage 에 저장된 토큰 가져오기
        // const token = localStorage.getItem("token"); 
        //2) 만약에 토큰이 없으면 함수 종료 
        // if(!token){return;}
        //3) 헤더에 표시할 로그인된 유저 아이디 가져오기 
        const response = await axios.get(
            "http://localhost:8080/api/member3/my/info", // 통신할 (스프링 컨트롤러 매핑) 주소 
             { withCredentials : true }  // {headers : {속성명 : 값}}
            // * axios 특징 : Content-Type : application/json 자동으로 설정됨 (객체 전송시)
            // 만약에 Content-Type이 json이 아닐 경우 명시한다. 
        );
        //4) 통신 결과 분기  
        const data = response.data; // 응답받은 데이터
        if(data != null && data != false){ // 응답 자료가 존재하면 
            setLogin(true); // 로그인 상태로 변경
            setUser(data); // 응답 받은 자료 (회원정보) 저장 
        }
        else { // 응답 자료가 존재하지 않으면
            setLogin(false); // 비로그인 상태로 변경
            setUser(null); // 회원 정보 초기화 
        }   
        
    }

    // [4] 헤더가 열리면 최초 1번 실행 , 로그인 상태(백엔드 검증해야한다.) 
    useEffect( ()=>{getMyInfo(); }, [] );

    // [5] 로그아웃 
    const logout = async()=>{
     
        // 1) axios 
        const response = await axios.get(
            "http://localhost:8080/api/member3/logout",
             { withCredentials : true } 

        );
        setLogin(false); // 로그아웃 상태로 변경
        alert('로그아웃 되었습니다.');
        location.href="/"; // 메인페이지 이동 

    }



    // js 삼항 연산자 , 조건 ? 참 : 거짓
    // js 단축평가 , 조건 && 실행문 , 조건이 참이면 실행문 거짓이면 생략 
    return (
        <div
        >
            <Link to="/"> 홈 </Link> |

            {/*비로그인메뉴*/}
            {login == false && (<>
            <Link to="/member/login"> 로그인 </Link> |
            <Link to="/member/signup"> 회원가입 </Link> |
            <Link to="/board"> 게시판 </Link> |
            </>)} 

             {/*로그인메뉴*/}
            {login == true && (<>
            <span>{user.mid} 님 </span> |
            <Link to="/member/page"> 내정보 </Link> |
            <Link to="/board/write"> 글쓰기 </Link> |
            <Link to="/board"> 게시판 </Link> |
            <Link to="/chat"> 채팅방 </Link> |
            <button onClick={logout}> 로그아웃 </button>
            </>)}

            <hr/>
        </div>
    )
}