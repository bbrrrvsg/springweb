...
public interface TodoRepository
    ...
    //2] 쿼리 메소드 : sql 직접 작성하지 않고 추상메소드 이름으로 쿼리 자동 생성 < 카멜표기법 >
        //2-1 : title으로 일치 조회 , 엔티티타입명 findBy필드명(타입매개변수)
    ...
    // select * from todo where title = :매개변수명 앞에 클론 이용하여 매개변수값 대입
    @Query(value = "selcet * from todo where title = :title",nativeQuery = true)
    TodoEntity query1(String title); // 여기 매개변수명 이랑 같아야함 :title
        //3-2 :
