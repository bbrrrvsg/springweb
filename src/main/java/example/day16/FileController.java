...
/*
    ...
    3. 스트림 : 데이터가 이동하는 흐름 , 스트림API / csv , 액셀 파일처리 / 네트워크  등등
    4. 버퍼 : 데이터가 이동하는 흐름간의 처리 속도를 일정하게 하기위한 임시 메모리 * 흐름 하는 순간에도 누군가의 기억 *
    5. 자바입출력 클래스
        1) FileInputStream
        2) ServletOutputStream 등등
    [ 스프링 업로드 구현 ] * 내장 업로드 객체 지원한다.
        1. MultipartFile 클래스 : 첨부파일(바이트) 매핑하는 인터페이스
        2. 사용법
            첨부파일1개 : MultipartFile 변수명
            첨부파일N개 : List<MultipartFile> 변수명

        3. content-Type : multipart/foem-data

*/
/*
    (1) 쿼리스트링 방식 , URL ? 변수명 = 값
    (2) 본문 방식 , URL , {"변수명" : 값} , content-type : application/json
    (3) 대용량(byte) 방식 , URL , form , content-type : mutipart/form-data
